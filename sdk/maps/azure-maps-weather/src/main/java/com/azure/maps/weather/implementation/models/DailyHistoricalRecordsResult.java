// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.maps.weather.implementation.models;

import com.azure.core.annotation.Fluent;
import com.fasterxml.jackson.annotation.JsonProperty;
import java.util.List;

/** The DailyHistoricalRecordsResult model. */
@Fluent
public final class DailyHistoricalRecordsResult {
    /*
     * Historical records for each requested day.
     */
    @JsonProperty(value = "results")
    private List<DailyHistoricalRecords> historicalRecords;

    /*
     * The is the link to the next page of the features returned. If it's the
     * last page, no this field.
     */
    @JsonProperty(value = "nextLink")
    private String nextLink;

    /**
     * Get the historicalRecords property: Historical records for each requested day.
     *
     * @return the historicalRecords value.
     */
    public List<DailyHistoricalRecords> getHistoricalRecords() {
        return this.historicalRecords;
    }

    /**
     * Set the historicalRecords property: Historical records for each requested day.
     *
     * @param historicalRecords the historicalRecords value to set.
     * @return the DailyHistoricalRecordsResult object itself.
     */
    public DailyHistoricalRecordsResult setHistoricalRecords(List<DailyHistoricalRecords> historicalRecords) {
        this.historicalRecords = historicalRecords;
        return this;
    }

    /**
     * Get the nextLink property: The is the link to the next page of the features returned. If it's the last page, no
     * this field.
     *
     * @return the nextLink value.
     */
    public String getNextLink() {
        return this.nextLink;
    }

    /**
     * Set the nextLink property: The is the link to the next page of the features returned. If it's the last page, no
     * this field.
     *
     * @param nextLink the nextLink value to set.
     * @return the DailyHistoricalRecordsResult object itself.
     */
    public DailyHistoricalRecordsResult setNextLink(String nextLink) {
        this.nextLink = nextLink;
        return this;
    }
}
