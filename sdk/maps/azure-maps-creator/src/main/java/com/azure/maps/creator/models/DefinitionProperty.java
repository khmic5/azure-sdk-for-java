// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.maps.creator.models;

import com.azure.core.annotation.Fluent;
import com.fasterxml.jackson.annotation.JsonProperty;

/** The DefinitionProperty model. */
@Fluent
public final class DefinitionProperty {
    /*
     * name of attribute
     */
    @JsonProperty(value = "name", required = true)
    private String name;

    /*
     * is attribute required
     */
    @JsonProperty(value = "required", required = true)
    private boolean required;

    /*
     * type of attribute
     */
    @JsonProperty(value = "type", required = true)
    private Object type;

    /**
     * Get the name property: name of attribute.
     *
     * @return the name value.
     */
    public String getName() {
        return this.name;
    }

    /**
     * Set the name property: name of attribute.
     *
     * @param name the name value to set.
     * @return the DefinitionProperty object itself.
     */
    public DefinitionProperty setName(String name) {
        this.name = name;
        return this;
    }

    /**
     * Get the required property: is attribute required.
     *
     * @return the required value.
     */
    public boolean isRequired() {
        return this.required;
    }

    /**
     * Set the required property: is attribute required.
     *
     * @param required the required value to set.
     * @return the DefinitionProperty object itself.
     */
    public DefinitionProperty setRequired(boolean required) {
        this.required = required;
        return this;
    }

    /**
     * Get the type property: type of attribute.
     *
     * @return the type value.
     */
    public Object getType() {
        return this.type;
    }

    /**
     * Set the type property: type of attribute.
     *
     * @param type the type value to set.
     * @return the DefinitionProperty object itself.
     */
    public DefinitionProperty setType(Object type) {
        this.type = type;
        return this;
    }
}
