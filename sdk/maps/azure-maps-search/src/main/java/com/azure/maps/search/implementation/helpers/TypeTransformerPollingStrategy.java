 ...
 *           kept
 */
public class TypeTransformerPollingStrategy<T, U> implements PollingStrategy<T, U> {
    private static final String DEFAULT_RESOURCE_LOCATION_HEADER = "Location";

    private final HttpPipeline httpPipeline;
    private final ObjectSerializer serializer;
    private final String operationLocationHeaderName;
    private final PollerType pollerType;

    public static enum PollerType {
        FORWARD,
        REVERSE
    };
     ...
     */
    public TypeTransformerPollingStrategy(HttpPipeline httpPipeline, PollerType pollerType) {
        this(httpPipeline, new DefaultJsonSerializer(), DEFAULT_RESOURCE_LOCATION_HEADER, pollerType);
    }
     ...
     */
    public TypeTransformerPollingStrategy(HttpPipeline httpPipeline, ObjectSerializer serializer,
                                            String operationLocationHeaderName, PollerType pollerType) {
        ...
        this.operationLocationHeaderName = operationLocationHeaderName != null ? operationLocationHeaderName
            : DEFAULT_RESOURCE_LOCATION_HEADER;
            ...
            .flatMap(binaryData -> PollingUtils.deserializeResponse(
                ...
                .map(pollResult -> {
                    ...
                    switch (response.getStatusCode()) {
                        case 200:
                            pollResult.setStatus(LongRunningOperationStatus.SUCCESSFULLY_COMPLETED);
                            break;
                        case 202:
                            pollResult.setStatus(LongRunningOperationStatus.IN_PROGRESS);
                            break;
                        default:
                            pollResult.setStatus(LongRunningOperationStatus.FAILED);
                    }
                ...
                .flatMap(status -> {
                    ...
                    // monos for conversion of the intermediate type
                    Mono<SearchAddressBatchResult> forwardMono = PollingUtils.deserializeResponse(binaryData,
                        serializer, new TypeReference<SearchAddressBatchResult>(){});
                    Mono<ReverseSearchAddressBatchResultPrivate> reverseMono = PollingUtils.deserializeResponse(binaryData,
                        serializer, new TypeReference<ReverseSearchAddressBatchResultPrivate>(){});
                    Mono<?> resultMono = (this.pollerType == PollerType.FORWARD) ? forwardMono : reverseMono;

                    Mono<T> finalMono = resultMono.flatMap(result -> {
                        if (result != null) {
                            // this needs to be reviewed
                            if (pollerType == PollerType.FORWARD) {
                                return Mono.just((T) Utility.toBatchSearchResult((SearchAddressBatchResult)result));
                            }
                            else {
                                return Mono.just((T) Utility.toBatchReverseSearchResult((ReverseSearchAddressBatchResultPrivate)result));
                            }
                        }
                        else {
                            return Mono.empty();
                    ...
                    return finalMono.map(value -> new PollResponse<>(status, value, retryAfter));
                })));
    }
        ...
        if (finalGetUrl == null) {
            ...
            return httpPipeline.send(request)
                ...
                .flatMap(binaryData -> {
                    ...
                    Mono<U> finalMono = resultMono.flatMap(result -> {
                        if (result != null) {
                            // this needs to be reviewed
                            if (this.pollerType == PollerType.FORWARD) {
                                return Mono.just((U) Utility.toBatchSearchResult((SearchAddressBatchResult)result));
                            }
                            else {
                                return Mono.just((U) Utility.toBatchReverseSearchResult((ReverseSearchAddressBatchResultPrivate)result));
                            }
                        }
                        else {
                           return Mono.empty();
