 ...
 * Tasks are commonly used to support MPI Tasks. In the MPI case, if any of the
 * subtasks fail (for example due to exiting with a non-zero exit code) the
 * entire multi-instance Task fails. The multi-instance Task is then terminated
 * and retried, up to its retry limit.
 */
public class MultiInstanceSettings {
     ...
     * coordination command line. The difference between common resource files
     ...
     * size is exceeded, the request will fail and the response error code will
     * be RequestEntityTooLarge. If this occurs, the collection of
    ...
    /**
     * Get the number of Compute Nodes required by the Task. If omitted, the default is 1.
     *
    ...
    /**
     * Set the number of Compute Nodes required by the Task. If omitted, the default is 1.
     *
    ...
    /**
     * Get the command line to run on all the Compute Nodes to enable them to coordinate when the primary runs the main Task command. A typical coordination command line launches a background service and verifies that the service is ready to process inter-node messages.
     *
    ...
    /**
     * Set the command line to run on all the Compute Nodes to enable them to coordinate when the primary runs the main Task command. A typical coordination command line launches a background service and verifies that the service is ready to process inter-node messages.
     *
